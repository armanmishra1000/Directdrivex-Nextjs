    ...
    <div className="lg:col-span-3 space-y-6 sm:space-y-8 lg:space-y-10">
      ...
      <div className="space-y-3 sm:space-y-4 lg:space-y-5">
        <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-extrabold text-slate-900 leading-tight">
          Secure File Sharing <span className="text-blue-600">Made Simple</span>
        </h1>
        <p className="text-lg sm:text-xl md:text-2xl font-medium text-slate-600 leading-relaxed">
          Transfer files up to <span className="font-bold text-blue-600">30GB</span> with bank-level security
        </p>
      </div>

      <div className="space-y-4 sm:space-y-5">
        <button className="bg-gradient-to-r from-blue-600 to-blue-700 text-white font-bold py-3 sm:py-4 px-6 sm:px-8 mb-3 rounded-xl shadow-lg hover:shadow-xl transition-all hover:-translate-y-1 text-base sm:text-lg w-full sm:w-auto">
          Claim Your 50GB Now - Free Forever
        </button>
        <div className="space-y-3 sm:space-y-4">
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl p-3 sm:p-4 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
              <div className="flex items-center space-x-2 sm:space-x-3">
                ...
                <div className="min-w-0 flex-1">
                  <span className="text-xs sm:text-sm font-semibold text-blue-800 block">🔥 Free 50GB Accounts Claimed</span>
                  <p className="text-xs text-blue-600">Limited time offer - Act fast!</p>
                </div>
              </div>
              <div className="text-center sm:text-right">
                <span className="text-base sm:text-lg font-bold text-blue-900">78%</span>
                <p className="text-xs text-blue-600">claimed</p>
              </div>
            </div>
            <div className="space-y-2 mt-3 sm:mt-4">
              <div className="w-full bg-blue-200 rounded-full h-2 sm:h-3 shadow-inner">
                <div className="bg-gradient-to-r from-blue-500 to-indigo-600 h-2 sm:h-3 rounded-full shadow-sm" style={{ width: '78%' }}></div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-3 sm:gap-4 lg:gap-6">
          {['256-bit Encryption', 'SOC2 Compliant', 'Zero-Knowledge', '99.9% Uptime'].map((text, i) => (
            <div key={text} className="flex items-center space-x-2">
              <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full flex items-center justify-center ${['bg-green-500', 'bg-blue-500', 'bg-purple-500', 'bg-emerald-500'][i]}`}>
